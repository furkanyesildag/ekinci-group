...
export default function ProjectCard({ project, className = '', imageAspect = 'aspect-video' }: Props) {
  return (
    <Link href={`/projeler/${project.slug}`}
      className={`group relative rounded-2xl overflow-hidden shadow-ambient hover:shadow-ambient-xl transition-all duration-500 hover:-translate-y-1 block ${className}`}>
